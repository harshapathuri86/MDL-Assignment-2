...
class State:
    def __init__(self, position, arrows, material, enemy_state, health):
        if (
            ...
            or (enemy_state not in ENEMY_STATE)
        ):
            raise ValueError
    ...
    if action_type == "ACTION_UP":
        ...
        for possibility in possibilities:
            if state.enemy_state == "D":
                choices.append(
                    (possibility[0] * PROBS["enemy"]["D"]["R"], possibility[1])
                )
                choices[-1][1].enemy_state = "R"
                choices.append(
                    (possibility[0] * PROBS["enemy"]["D"]["D"], possibility[1])
                )
                ...
                if state.position in ["E", "C"]:
                    ...
                    choices.append(
                        (possibility[0] * PROBS["enemy"]["R"]["D"], possibility[1])
                    )
                choices[-1][1].enemy_state = "D"
                choices.append(
                    (possibility[0] * PROBS["enemy"]["R"]["R"], possibility[1])
                )
                choices[-1][1].enemy_state = "R"
...
def value_iteration():
    utilities = np.zeros(
        (POSITIONS, ARROWS_RANGE, MATERIAL_RANGE, NUM_STATES, HEALTH_RANGE)
    )
    policies = np.full(
        (POSITIONS, ARROWS_RANGE, MATERIAL_RANGE, NUM_STATES, HEALTH_RANGE),
        -1,
